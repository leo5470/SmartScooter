...
// 匿名使用者
export const anonymous_user = new User(-1, init_location, "", "", 0, "", "");

// 匿名 session
const anonymous_session = "";

export const proxt_data = proxy(JSON.parse(localStorage.getItem(config.local_storage_keyname) as string) as Iproxy || { current_location: init_location, current_user: anonymous_user, current_session: anonymous_session, current_order: null , is_admin:false });
